...
class CleanupScanner:
    ...
    def scan_package_cache(self) -> ScanResult:
        ...
        return ScanResult(
            category="Package Cache", size_bytes=total_size, count=len(files), items=files
        )
    ...
    def scan_orphaned_packages(self) -> ScanResult:
        ...
        return ScanResult(
            category="Orphaned Packages", size_bytes=size_bytes, count=len(packages), items=packages
        )
    ...
    def scan_temp_files(self, days_old: int = 7) -> ScanResult:
        ...
        return ScanResult(
            category="Temporary Files", size_bytes=total_size, count=len(files), items=files
        )
    ...
    def scan_logs(self, min_size_mb: int = 100, days_old: int = 7) -> ScanResult:
        ...
        return ScanResult(category="Old Logs", size_bytes=total_size, count=len(files), items=files)
