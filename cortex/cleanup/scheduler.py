...
class CleanupScheduler:
    ...
    def enable_schedule(
        self, interval: ScheduleInterval = ScheduleInterval.WEEKLY, safe_mode: bool = True
    ) -> dict[str, Any]:
        """
    ...
    def _setup_systemd_timer(
        self, interval: ScheduleInterval, safe_mode: bool = True
    ) -> dict[str, Any]:
        """
...
"""
    ...
    def _setup_cron(self, interval: ScheduleInterval, safe_mode: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            mode_flag = "--safe" if safe_mode else "--force"
            cron_command = (
                f"{cron_schedule} /usr/bin/env cortex cleanup run {mode_flag} --yes {CRON_TAG}"
            )
            ...
            lines = [line for line in current_crontab.splitlines() if CRON_TAG not in line]
    ...
    def _remove_cron(self) -> None:
        ...
        try:
            ...
            lines = [line for line in result.stdout.splitlines() if CRON_TAG not in line]
