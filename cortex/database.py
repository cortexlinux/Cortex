...
class SuggestionDatabase:
    ...
    def get_stack(self, stack_id: str) -> Optional[Dict]:
    ...
    def detect_intent_from_keywords(self, tokens: List[str]) -> Optional[str]:
        """
    ...
    def detect_language_from_keywords(self, tokens: List[str]) -> Optional[str]:
    ...
    def get_stacks_by_intent_and_language(self, intent_id: str, lang_id: str) -> List[Dict]:
        ...
        intent_stacks = set(s["id"] for s in self.get_stacks_by_intent(intent_id))
        lang_stacks = set(s["id"] for s in self.get_stacks_by_language(lang_id))
        matching_ids = intent_stacks & lang_stacks
        return [self._stacks_by_id[sid] for sid in matching_ids]
    
    def filter_gpu_stacks(self, stacks: List[Dict], has_gpu: bool = True, 
                          vendor: str = "nvidia") -> List[Dict]:
        """Filter stacks by GPU requirements."""
        if has_gpu:
            return [s for s in stacks 
                    if not s.get("requires_gpu") or s.get("gpu_vendor") == vendor]
