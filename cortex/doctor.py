...
class SystemDoctor:
    ...
    def _print_check(
        self, status: str, message: str, suggestion: Optional[str] = None
    ) -> None:
        """
    ...
    def _check_python(self) -> None:
        ...
        if sys.version_info >= (3, 10):
            ...
            self._print_check(
                "WARN",
                f"Python {version} (3.10+ recommended)",
    ...
    def _check_dependencies(self) -> None:
        """Check packages from ACTUAL requirements.txt."""
        missing = []
        ...
        if shutil.which("nvcc"):
            try:
                ...
                if result.returncode == 0 and "release" in result.stdout:
                    version_line = (
                        result.stdout.split("release")[1].split(",")[0].strip()
                    )
    ...
    def _print_summary(self) -> None:
        ...
        if self.passes:
            table.add_row(
                "[green]✓ Passed[/green]", f"[green]{len(self.passes)}[/green]"
            )
        if self.warnings:
            table.add_row(
                "[yellow]⚠ Warnings[/yellow]", f"[yellow]{len(self.warnings)}[/yellow]"
            )
        if self.failures:
