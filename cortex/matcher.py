...
class IntentMatcher:
    ...
    STOP_WORDS = {'for', 'with', 'and', 'the', 'a', 'an', 'to', 'in', 'on', 'of', 'i', 'want', 'need', 'install', 'setup', 'set', 'up'}
    ...
    def _detect_context(self, tokens: List[str], full_query: str) -> Tuple[Optional[str], Optional[str]]:
        """
                ...
                if token in kw or kw in token:
                    for sid in stack_ids:
                        if sid not in candidates:
                            stack = self.db.get_stack(sid)
                            if stack:
                                score = self.SCORE_KEYWORD_PARTIAL + self._score_keyword_match(tokens, stack)
                                candidates[sid] = max(candidates.get(sid, 0), score)
