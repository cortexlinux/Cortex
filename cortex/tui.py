...
class SuggestionTUI:
    ...
    def render_results(self, query: str) -> Table:
        ...
        for i, item in enumerate(self.results):
            ...
            name = item["name"]
            desc = item["description"][:42] + "..." if len(item["description"]) > 45 else item["description"]
    ...
    def render_preview(self) -> Panel:
        ...
        if preview.get('apt_packages'):
            ...
            content += f"[bold]APT Packages:[/bold]\n"
            for pkg in pkgs[:8]:
                content += f"  • {pkg}\n"
            if len(pkgs) > 8:
    ...
    def run_interactive(self, initial_query: str = "") -> Optional[Dict]:
        ...
        try:
            ...
            max_name_width = max(len(item["id"]) for item in self.results[:viewport_size + 5]) if self.results else 16
            ...
            def get_text():
                ...
                for i, item in enumerate(visible_results):
                    ...
                    if actual_index == self.selected_index:
                        lines.append(f"<style bg='ansiblue'><b>{prefix} {name_padded}</b> {desc}</style>")
                    else:
                ...
                lines.append("")
                lines.append("<ansigray>[↑↓ to select, Enter to install, Tab for details, q to quit]</ansigray>")
    ...
    def run_simple(self, query: str) -> Optional[Dict]:
...
def run_suggestions(query: str = "", db_path: Optional[str] = None, interactive: bool = True) -> Optional[Dict]:
    """
...
def print_suggestions(query: str, db_path: Optional[str] = None, limit: int = 5):
