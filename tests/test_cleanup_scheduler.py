...
class TestScheduleConfig:
    ...
    def test_to_dict(self):
        ...
        config = ScheduleConfig(
            enabled=True, interval=ScheduleInterval.DAILY, safe_mode=False, last_run=1234567890.0
        )
    ...
    def test_from_dict(self):
        ...
        data = {"enabled": True, "interval": "monthly", "safe_mode": True, "last_run": 9876543210.0}
...
class TestCleanupScheduler:
    ...
    def test_save_and_load_config(self, scheduler):
        ...
        config = ScheduleConfig(enabled=True, interval=ScheduleInterval.DAILY, safe_mode=True)
    ...
    def test_get_status_disabled(self, scheduler):
        ...
        with (
            patch.object(scheduler, "_check_systemd_timer", return_value=False),
    ...
    def test_get_status_enabled(self, scheduler):
        ...
        with (
            patch.object(scheduler, "_check_systemd_timer", return_value=True),
            patch.object(scheduler, "_check_cron", return_value=False),
        ):
            status = scheduler.get_status()
    ...
    def test_enable_schedule_systemd_success(self, mock_run, scheduler, tmp_path):
        ...
        with patch.object(Path, "home", return_value=tmp_path):
            result = scheduler.enable_schedule(interval=ScheduleInterval.WEEKLY, safe_mode=True)
